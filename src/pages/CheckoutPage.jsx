...
import { payWithMoMo } from '@utils/momoPayment';
...
const CheckoutPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Backend expect: "COD", "CARD", "MOMO" (UPPERCASE) hoặc "cod", "card", "momo" (lowercase)
            // Theo ví dụ API docs, backend chấp nhận UPPERCASE
            let backendPaymentMethod = paymentMethod.toUpperCase(); // COD, CARD, MOMO
            ...
            // Handle MoMo payment
            if (paymentMethod === 'momo') {
                const orderId = response.id || response.data?.id || response.order_id;
                const orderCode = response.order_code || response.data?.order_code || `ORD${Date.now()}`;

                if (!orderId) {
                    throw new Error('Không nhận được Order ID từ server');
                }

                // Clear cart ngay sau khi tạo order thành công
                // Vì order đã được tạo, dù thanh toán có thành công hay không
                clearCart();

                try {
                    const orderInfo = `Thanh toán đơn hàng ${orderCode}`;
                    await payWithMoMo(orderId, orderInfo);
                } catch (momoError) {
                    console.error('❌ Error creating MoMo payment:', momoError);
                    showToast(momoError.message || 'Có lỗi xảy ra khi tạo thanh toán MoMo. Vui lòng thử lại.', 'error');
                    setSubmitting(false);
                }
            } else if (paymentMethod === 'qr' || paymentMethod === 'card') {
                // Create QR payment
        ...
        <>
            ...
            <div className="min-h-screen bg-gray-50 pb-6">
                <div className="max-w-6xl mx-auto px-4 py-4">
                    <h1 className="text-xl font-bold mb-4 text-gray-900">THANH TOÁN</h1>
                        ...
                        <div className="grid lg:grid-cols-3 gap-4">
                            ...
                            <div className="lg:col-span-2 space-y-3">
                                ...
                                <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                                    <h2 className="text-sm font-black uppercase text-gray-900 tracking-tight mb-3 pb-2 border-b-2 border-gray-900">
                                        THÔNG TIN GIAO HÀNG
                                    </h2>
                                ...
                                <div className="space-y-2.5 mt-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            autoComplete="name"
                                            className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 transition-colors ${touched.fullName && errors.fullName
                                        ...
                                        {touched.fullName && errors.fullName && (
                                            <p id="fullName-error" className="mt-0.5 text-xs text-red-600" role="alert">
                                                {errors.fullName}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            autoComplete="tel"
                                            className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 transition-colors ${touched.phone && errors.phone
                                                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                                    : 'border-gray-300 focus:border-[#ff6600] focus:ring-[#ff6600]'
                                                }`}
                                            required
                                        ...
                                        {touched.phone && errors.phone && (
                                            <p id="phone-error" className="mt-0.5 text-xs text-red-600" role="alert">
                                                {errors.phone}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        <label htmlFor="email" className="block text-xs font-medium text-gray-700 mb-0.5">
                                            Email <span className="text-gray-400 text-xs font-normal">(Tùy chọn)</span>
                                        </label>
                                        <input
                                            id="email"
                                            type="email"
                                            ...
                                            autoComplete="email"
                                            className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 transition-colors ${touched.email && errors.email
                                                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                                    : 'border-gray-300 focus:border-[#ff6600] focus:ring-[#ff6600]'
                                                }`}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 transition-colors ${touched.address && errors.address
                                                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                                    : 'border-gray-300 focus:border-[#ff6600] focus:ring-[#ff6600]'
                                                }`}
                                            required
                                        ...
                                        {touched.address && errors.address && (
                                            <p id="address-error" className="mt-0.5 text-xs text-red-600" role="alert">
                                                {errors.address}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#ff6600] focus:ring-1 focus:ring-[#ff6600] bg-white disabled:bg-gray-100 disabled:cursor-not-allowed"
                                        ...
                                        <div>
                                            ...
                                            {districts.length > 0 ? (
                                                <select
                                                    ...
                                                    className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:border-[#ff6600] focus:ring-1 focus:ring-[#ff6600] bg-white disabled:bg-gray-100 disabled:cursor-not-allowed"
                                                ...
                                                <input
                                                    ...
                                                    autoComplete="address-level2"
                                                    className="w-full border border-gray-300 rounded-lg px-2 py-2 text-sm focus:outline-none focus:border-[#ff6600] focus:ring-1 focus:ring-[#ff6600] disabled:bg-gray-100 disabled:cursor-not-allowed"
                                                    disabled={!formData.province}
                                                />
                                            )}
                                        </div>
                                    ...
                                    <div className="pt-1">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                className="w-4 h-4 text-[#ff6600] rounded border-gray-300 focus:ring-[#ff6600]"
                                            />
                                            <span className="text-xs text-gray-700">Sử dụng địa chỉ giao hàng mặc định</span>
                                        </label>
                                    </div>
                                    </div>
                                </div>
                            ...
                            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                                ...
                                <div className="space-y-2 mt-3">
                                        ...
                                        <label className={`flex items-center p-3 border-2 rounded cursor-pointer transition-all ${paymentMethod === 'momo' ? 'border-[#ff6600] bg-orange-50' : 'border-gray-300 hover:border-gray-400'}`}>
                                            <input
                                                type="radio"
                                                name="payment"
                                                value="momo"
                                            ...
                                            />
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm text-gray-900">Thanh toán qua MoMo</span>
                                ...
                                <div className="bg-white rounded-lg shadow-sm border border-gray-200 sticky top-4 overflow-hidden">
                                    ...
                                    <div className="p-4">
                                        ...
                                        <button
                                            type="submit"
                                            disabled={submitting}
                                            className="w-full bg-[#ff6600] text-white py-3.5 rounded-lg font-bold uppercase tracking-wide hover:bg-orange-700 transition-all disabled:bg-gray-400 disabled:cursor-not-allowed shadow-md hover:shadow-lg text-sm"
                                        >
                                            {submitting ? (
